...
import { addDoc, collection, doc, getDocs, serverTimestamp, Timestamp } from "firebase/firestore";
import React, { useCallback, useEffect, useState } from 'react';
import { Alert, FlatList, Image, Modal, ScrollView, StyleSheet, Text, TextInput, TouchableOpacity, TouchableWithoutFeedback, useWindowDimensions, View } from 'react-native';
...
const PosScreen = () => {
  ...
  useEffect(() => {
    ...
    if (appliedDiscount) {
        const calculateDiscount = (sub: number, percent: number) => sub * (percent / 100);
        if (appliedDiscount.type === 'Entire Order') {
            newDiscountAmount = calculateDiscount(newSubtotal, appliedDiscount.percent);
        } else { // Category or Food specific discounts applied globally
            ...
            newDiscountAmount = calculateDiscount(discountedSubtotal, appliedDiscount.percent);
        }
    }

    const finalSubtotal = orderItems.reduce((acc, item) => acc + (item.food.price * item.quantity), 0);
    const totalItemDiscount = orderItems.reduce((acc, item) => {
        const itemTotal = item.food.price * item.quantity;
        return acc + (item.discount ? itemTotal * (item.discount / 100) : 0);
    }, 0);

    const newTax = (finalSubtotal - totalItemDiscount) * 0.1;
    const newServiceCharge = (finalSubtotal - totalItemDiscount) * 0.1;
    const newTotal = finalSubtotal - totalItemDiscount + newTax + newServiceCharge - newDiscountAmount;

    setSubtotal(finalSubtotal);
    setDiscountAmount(totalItemDiscount + newDiscountAmount);
    setTax(newTax);
  ...
  const addToOrder = (food: Food) => {
    // Always add as a new item to allow for separate discounts/notes
    const newItem: OrderItemType = {
        id: `${food.id}-${Date.now()}`, // Unique ID
        food,
        quantity: 1,
    };
  ...
  const handleUpdateItem = () => {
      if (selectedOrderItem) {
          ...
          if (!isNaN(qty) && qty > 0) {
              ...
              setOrderItems(prevItems => prevItems.filter(item => item.id !== selectedOrderItem.id));
          }
          closeItemEditor();
      }
  }
  ...
  const handlePay = async () => {
    if (orderItems.length === 0) return Alert.alert("Empty Order", "Please add items to the order.");
    try {
  ...
  const handleLogout = () => {
    if (orderItems.length > 0) {
      Alert.alert("Unable to Logout", "Please clear the current order before logging out.");
      return;
    }
  ...
  const orderSidebar = (
    <View style={[styles.orderContainer, isSystemLocked && styles.disabledView]}>
      ...
      <View style={styles.actionButtonsContainer}>
        ...
        <TouchableOpacity style={[styles.actionButton, styles.payButton, isSystemLocked && styles.disabledButton]} onPress={handlePay} disabled={isSystemLocked}><Text style={styles.payButtonText}>PAY</Text></TouchableOpacity>
      </View>
    </View>
  );

  const renderSidebar = () => (
    <Modal animationType="fade" transparent={true} visible={isSidebarVisible} onRequestClose={() => setIsSidebarVisible(false)}>
        <TouchableOpacity style={styles.sidebarOverlay} onPress={() => setIsSidebarVisible(false)} />
        <View style={styles.sidebar}>
            ...
            <TouchableOpacity
                ...
                <Text style={[styles.sidebarLink, !isSystemLocked && styles.disabledText]}>Staff</Text>
            </TouchableOpacity>
        </View>
    </Modal>
  )
  ...
  return (
    <SafeAreaView style={styles.container}>
        ...
        <View style={styles.header}>
            <TouchableOpacity onPress={() => setIsSidebarVisible(true)}><Ionicons name="menu-outline" size={32} color="white" /></TouchableOpacity>
            <View>
